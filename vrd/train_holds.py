...
classes_2 = set([
    '/m/05_5p_0', '/m/0hg7b', '/m/0dv5r', '/m/0dv9c', '/m/0342h',
...
def create_train_data(args):
    ...
    df_pos = df_vrd.loc[df_vrd.RelationshipLabel=='holds'].copy()
    ...
    print('num holds images:', len(df_vrd.loc[df_vrd.RelationshipLabel=='holds'].ImageID.unique()))
...
def get_train_data(args):
    ...
    print('num pos samples:', df_pos.shape)

    df_neg = shuffle(pd.read_csv(args.neg_sample_fn)).iloc[:60000]
    df_neg.RelationshipLabel = 0
    #df_neg.iloc[0].RelationshipLabel = 'xxx'
    print('num neg samples:', df_neg.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generate', type=str, default='holds/df_neg.csv')
    parser.add_argument('--train', action='store_true')
    parser.add_argument('--neg_sample_fn', type=str, default=None)
    parser.add_argument('--model_file', type=str, default='holds/cat.model')
