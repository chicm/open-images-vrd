...
from train_wears import classes_1, classes_2, classes, add_features, parallel_apply
...
def fast_get_prediction_string(test_row):
    ...
    cur_x_test['RelationshipLabel'] = pd.Series(pred_rel).map(lambda x: 'wears' if x == 1 else 'none')
