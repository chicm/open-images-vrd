...
from train_hits import add_features, parallel_apply
...
def fast_get_prediction_string(test_row):
    ...
    dets_c1 = [x for x in dets if x[0] in classes_1]
    dets_c2 = [x for x in dets if x[0] in classes_2]

    if len(dets_c1) < 1 or len(dets_c2) < 1:
        return ''
    ...
    cur_x_test = cur_x_test.nlargest(30, 'sort_score').copy()
